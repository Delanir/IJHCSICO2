...
table_asia <- merge(x = cluster_drivers, y = mycsvfile_asia, by.y = "driver_id",by.x = "DriverID")
table_europe <- merge(x = cluster_drivers, y = mycsvfile, by.y = "driver_id",by.x = "DriverID")
table_usa <-merge(y = mycsvfile_us, x= cluster_drivers, by.y = "driver_id",by.x = "DriverID")

```

```{r, echo=FALSE}
table_europe$driver_type_ordered <- ordered(table_europe$Cluster, levels = c(1, 4, 2, 3))
table_asia$driver_type_ordered <- ordered(table_asia$Cluster, levels = c(1, 4, 2, 3))
table_usa$driver_type_ordered <- ordered(table_usa$Cluster, levels = c(1, 4, 2, 3))
...
```{r,echo=FALSE}
m<-with(table_europe,prop.table(table(colour,driver_type_ordered)))
m
sum(m)
```
...
```{r,echo=FALSE}
m<-with(table_asia,prop.table(table(colour,driver_type_ordered)))
m
