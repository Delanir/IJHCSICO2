...
This paper is structured as follows. Section \ref{sec:background} provides background research on eco-driving games, training applications, crowdsourcing and games with a purpose. Section \ref{sec:platform} describes the iCO$_2$ simulation platform, which extends our previous version of iCO$_2$ \citep{prendingeroliveira2014} with a quest system and upgrade functions. 
On Section \ref{sec:campaign} we provide insights obtained from the data collected from a user acquisition campaign. Besides overall statistics (presented in Section \ref{subsec:usage_stats}), we try to find patterns in the driving behavior using clustering methods (Section \ref{subsec:clustering}), we also mention other behavioral data insights. Moreover we test, and discuss whether our hypothesis in which iCO$_2$ players tend to become better at eco-driving (by considering smooth acceleration and brakings), the longer they play, on Section \ref{subsec:eco-friendliness_over_time}.
Section \ref{sec:conclusions} concludes the paper, by summarizing and discussing the most relevant results while paving the way for future work.
